...
if (!instance_exists(obj_randomsecret))
{
	if (secret)
	{
		...
		if (global.panic)
		{
			...
			savedpanicpos = fmod_event_instance_get_timeline_pos(global.panicmusicID);
			fmod_event_instance_set_paused(global.panicmusicID, true);
		}
	}
	else if (secretend)
	{
		...
		if (global.panic)
		{
			fmod_event_instance_set_timeline_pos(global.panicmusicID, savedpanicpos);
