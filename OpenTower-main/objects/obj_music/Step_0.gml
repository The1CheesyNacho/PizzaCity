...
if (fmod_event_instance_is_playing(kidspartychaseID) && instance_exists(obj_pause) && !obj_pause.pause && !instance_exists(obj_monster))
{
	...
	fmod_event_instance_set_paused(global.panicmusicID, savedpanicpause);
}
...
if (global.panic)
{
	if (!panicstart)
	{
		...
		{
			fmod_event_instance_play(global.panicmusicID);
			fmod_event_instance_set_paused(global.panicmusicID, false);
			fmod_event_instance_set_parameter(global.panicmusicID, "state", 0, true);
		}
		if (music != -4)
		{
		...
		{
			exitmusic = true;
			fmod_event_instance_stop(global.panicmusicID);
	...
	else if (fmod_event_instance_is_playing(global.panicmusicID))
	{
		if (!global.lap)
		{
			if (global.fill <= 672)
				fmod_event_instance_set_parameter(global.panicmusicID, "state", 1, true);
		}
		else
			fmod_event_instance_set_parameter(global.panicmusicID, "state", 2, true);
	}
}
else
{
	panicstart = false;
	fmod_event_instance_stop(global.panicmusicID, true);
}
var _found = false;
with (obj_totem)
{
...
if (exitmusic && fmod_event_instance_is_playing(global.panicmusicID))
	fmod_event_instance_stop(global.panicmusicID, true);
